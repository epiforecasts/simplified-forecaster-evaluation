...
\begin{align*}
  ...
  r_t &= \left(\mathcal{H}[a](r_{t-1} > 0) \xi_{+} + \mathcal{H}(r_{t-1} \le 0) \xi_{-}\right)r_{t-1} + \epsilon_t  \\
...
Breaking down the relative weighted interval score by forecast interval we observe that the majority of the difference in overall scores is likely driven by differences in the outer intervals [b][c][d][e]and in particular the tails of the forecast (Figure \@ref(fig:plot-coverage) b).
...
Over the 6 months of the study period, we found that our surrogate model produced forecasts that were visually similar to those from the forecast hub ensemble though with greater uncertainty[f][g].
In a subset of example locations, we observed some variation in performance across locations, that the ensemble appeared biased toward underprediction, and the surrogate model appeared slightly biased toward overprediction[h].
...
That being said no forecast hub has switched to these alternative ensemble designs for their operational forecast implying they do not think the evidence base is strong enough for them to be used by forecast consumers and hence the median of all submitted forecasts remains the community-suggested default ensemble option and a sensible target for our study. 
...
[a]Isn't the indicator function with 1(.) just more common and potentially easier to read?
[b]I would be very surprised if that were true. The reason is that the outer intervals get tiny weights in the WIS and performance usually is more strongly influenced by the inner intervals.
[c]This hangs on if the output from score is already weighted or not which I was not clear on?
[d]Which was my understanding of the implementation (weighting happens in score and summarise_score is just an averaging?)
...
[e]Aah I was just looking at Figure 3c here (I think it needs to be 3b). Oooh ok I think I now know what you mean. I would think that Figure 3b) is misleading. For every interval x, this is the score from the surrogate over the score for the ensemble for that interval x, right? Two reasons it's misleading: 
...
[f]WIS decompositions would be nice to look at. Although I guess can be left to future Sam (or never Sam)
[g]Suggest add to slack as a suggestion for discussion there (close once done please).
[h]As Johannes mentioned a while ago: It is a priori unclear what the "correct" distribution of overprediction, underprediction and dispersion is. So I think it is technically not correct to talk about a bias towards over / underprediction. Adding here as a comment because it might be relevant throughout. We could look at the bias value from scoringutils.
