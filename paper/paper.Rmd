...
However, ensemble forecasts have a range of potential downsides. The most obvious is their considerable cost to produce as they typically require contributions from multiple independent teams, the development of several models, and a centralised group to run the ensembling project. Forecasts from these ensembles can be hard to interpret as it is difficult to link them to the underlying model assumption.
...
Our previous methodology also requires significant computational resources running for an hour on a 16-core machine when producing forecasts for the ECDC forecasting hub.[a][b] 
...
In this study, we define an initial attempt at a surrogate model to replicate the observed behaviour of current multi-team forecast ensembles whilst maintaining a set of clear assumptions and summarising its implementation, with a focus on minimal resource use and reproducibility, as a GitHub Actions workflow. 
We briefly outline some of the summary statistics it produces and their epidemiological relevance. 
We then evaluate its real-time performance in comparison to the ECDC forecast hub ensemble by visualising forecasts, using the weighted interval score [@REF], a commonly used proper scoring rule, and quantifying the empirical coverage of the forecasts produced.
...
\begin{align*}
  ...
  r_t &= \left(\mathcal{H}[c](r_{t-1} > 0) \xi_{+} + \mathcal{H}(r_{t-1} \le 0) \xi_{-}\right)r_{t-1} + \epsilon_t  \\
...
`r relative_wis_thresholds[, better_than]` of surrogate forecasts scored better than the comparable ensemble forecast, `r relative_wis_thresholds[, fifty_percent]` performed within 50% of the comparable ensemble forecast, and `r relative_wis_thresholds[, more_than_100_percent]` scored performed worse than 100% more than the comparable ensemble forecast.
...
Overall the surrogate model was relatively well calibrated at the 30%, 60% and 90% credible intervals whilst the ensemble model was in general poorly calibrated with calibration becoming worse at larger credible intervals.
...
Stratifying calibration by quantile and forecast horizon the ensemble forecast was overly precise at all horizons for quantiles larger than the median whilst being comparably well calibrated for intervals below the median (Figure \@ref(fig:plot-coverage) c).
...
Breaking down the relative weighted interval score by forecast interval we observe that the majority of the difference in overall scores is likely driven by differences in the outer intervals [d][e][f][g][h]and in particular the tails of the forecast (Figure \@ref(fig:plot-coverage) b).
...
In a subset of example locations, we observed some variation in performance across locations, that the ensemble appeared biased toward underprediction, and the surrogate model appeared slightly biased toward overprediction[i].
Evaluating the relative performance of the surrogate model compared to the ECDC forecast hub ensemble we found that the mean performance was substantially worse, which was driven by a relatively small subset of forecasts.
...
Performance varied by location and forecast date with the surrogate model performing worse in the first part of 2022 which may have been linked to increasing incidence rates across forecast locations linked to BA.2.
...
In general, the relative performance of the surrogate model was stable across forecast horizons, unlike our previous methods which rapidly became less performant, though the distribution of relative performance did have an increasingly heavy right tail as the forecast horizon increased indicating a greater share of forecasts performing very poorly in comparison to the hub ensemble.
The forecast hub ensemble was poorly calibrated, particularly at longer forecast horizons and bigger credible intervals whereas the surrogate model was substantially better calibrated for all targets though it tended to overcover at large intervals.
...
Our study benefits from having been conducted using forecasts produced in real-time, rather than retrospectively, and submitted to an independent forecast research hub (though we note the overlap between authors on this study and the ECDC forecast hub).
...
However, if we start from the view that the forecast hub ensemble has traits that are desirable for use by policy-makers (i.e robustness and good average performance), which can be found widely in the literature, then our approach makes sense and may be optimal for producing a "good" forecast.
Our focus on replicating the performance of the hub ensemble is also useful as it highlights some of the emergent behaviour of the ensemble captured in our assumptions, such as auto-correlation across time points, and the growth rate tending towards zero as the forecast horizon increases.
...
We considered alternative methods of forecast evaluation that would be robust to this potential source of bias but choose to stick closely to the methodology used by the ECDC forecast hub as these choices inform the development of submitted models and so are key to our findings.
...
However, the CDC [@REF], ECDC [@REF], and Germany/Poland [@REF] forecasting hubs have published a range of evaluations of forecasts submitted to their platforms and the relative performance of their ensembles. 
In general, these studies have struggled to draw general conclusions about the structural assumptions of forecast models they consider "good" (generally they have defined this as minimising the weighted interval score as done in this study) and comparatively little attention has been paid to coverage.
The CDC forecast hub identified the top 5 performing models and noted the structural assumptions they made [@REF].
...
The CDC forecasting hub also evaluated a range of other ensemble approaches, such as inverse weighted interval score weighting, unweighted ensembles of a selection of models based on recent performance, and mean ensembling [@REF].
...
That being said no forecast hub has switched to these alternative ensemble designs for their operational forecast suggesting they do not think the evidence base is strong enough for them to be used by forecast consumers and hence the median of all submitted forecasts remains the community-suggested default ensemble option and a sensible target for our study. 
...
Lastly, here we have only explored a surrogate for an ensemble for a single disease and a limited set of locations meaning our findings are difficult to generalise.
...
[a]For this to be really informative, you would need to specify the CPU clock speed. There are huge disparities between the most recent 16-core machines and older ones.
[b]Agree it could/should list the Azure type and clock speed but disagree it brings no information as the comparison is to a 2 core CI process available in 2022 (i.e when the paper is written). 

I think perhaps adding this detail could be a bit overwhelming for an introduction but worth adding to try it out
[c]Isn't the indicator function with 1(.) just more common and potentially easier to read?
[d]I would be very surprised if that were true. The reason is that the outer intervals get tiny weights in the WIS and performance usually is more strongly influenced by the inner intervals.
[e]This hangs on if the output from score is already weighted or not which I was not clear on?
[f]Which was my understanding of the implementation (weighting happens in score and summarise_score is just an averaging?)

https://epiforecasts.io/scoringutils/reference/score_quantile.html

Perhaps there is an implementation nuance? I agree I was surprised by the magnitude of this difference.
[g]Aah I was just looking at Figure 3c here (I think it needs to be 3b). Oooh ok I think I now know what you mean. I would think that Figure 3b) is misleading. For every interval x, this is the score from the surrogate over the score for the ensemble for that interval x, right? Two reasons it's misleading: 
1) I think the values that you are dividing just become very small and therefore what you get is unstable, because the denominator gets smaller and smaller
2) the outer values don't influence the average a lot, because you first add the values for the individual intervals up (to get one WIS for ensemble and surrogate) and then you divide these values. The plot suggests that you divide the values for all the quantiles and then average to get your relative score
[h]That sounds like a wording/interpretation issue vs a change the plot issue? 

Do you have suggestions for that wording or if suggesting the plot is changed what it should be changed to?
[i]As Johannes mentioned a while ago: It is a priori unclear what the "correct" distribution of overprediction, underprediction and dispersion is. So I think it is technically not correct to talk about a bias towards over / underprediction. Adding here as a comment because it might be relevant throughout. We could look at the bias value from scoringutils.
