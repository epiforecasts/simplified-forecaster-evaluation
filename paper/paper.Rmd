...
These approaches have drawn heavily on the experience of other domains, for example, climate modelling [@IPCCreport], where ensembles of both multiple models and from multiple teams have a long history of providing superior forecasts and being trusted by policy-makers.
...
exclusions <- metadata |>
  DT(, .(forecast_date, location, anomaly)) |>
  unique() |>
...
forecast_dates <- metadata |>
  DT(, unique(forecast_date))

# Models per forecast date
models_per_forecast <- metadata |>
  summarise_forecasts_by(by = "forecast_date")
...
locs_per_model_single_date <- metadata |>
    summarise_forecasts_by(
      var = "location", by = c("model", "forecast_date")
    ) |>
...
models_per_loc_date <- metadata |>
  summarise_forecasts_by(var = "model", by = c("forecast_date", "location"))
...
forecast_dates_by_models <- metadata |>
  summarise_forecasts_by(var = "forecast_date", by = "model") |>
  DT(order(n)) |>
...
In total `r nrow(metadata)` forecasts were made across all locations, with unique `r forecast_dates` forecast dates and `r models` independent forecast models (including the European hub baseline model).
