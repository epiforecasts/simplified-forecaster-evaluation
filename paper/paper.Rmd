...
The experience of other domains has inspired these approaches, for example, climate modelling [@IPCCreport], where ensembles of both multiple models and from multiple teams have a long history of providing forecasts that stakeholders trust.
...
Little progress has so far been made in mitigating these downsides or in improving access to the high-quality and robust forecasts they seek to generate to geographies without coverage or for other infectious diseases. 
...
In the context of epidemioloigcal models non-parametric emulation has been used to allow the rapid exploration of the parameter space of complex models that would otherwise be resource prohibative [@Iskauskas2022-fz; @Charles2022-ds].
Replicating forecasts from resource intensive models may also be a promising avenue of future research. 
However, as an approach it may be less useful for resolving some of the issues of multi-team and multi-model forecasts as it does not provide interpretability, key for stakeholder take-up, it is not clear how the methods commonly used perform out of sample, and emulating a quantile based forecast is an unadressed methodological challenge as far as we are aware.
...
In order to achieve these aims, we evaluate an initial attempt at developing a surrogate model to replicate the observed behaviour of current multi-team forecast ensembles based on a set of clear assumptions.
...
The ensemble forecast was constructed by taking the median of all predictive quantiles without the exclusion of any validly submitted forecast (where validity was defined as passing minimal formatting checks by the hub team and timely submission) [@Sherratt2022-ue]. 
...
This resource usage is likely beyond the capacity of many interested in having access to state of the art short-term forecasts of infectious disease.
...
Finally, the ensemble forecast tended to produce sharper forecasts and have a tendency toward under vs over predicting.
...
Based on these observations (summarised in Table \@ref(tab:observations)), here we define a model with similar, but simplified, epidemiological characteristics to our previous approaches to model-based forecasting [@Bosse2022-hm] to produce an ensemble-like performance without sacrificing interpretability and with a lower cost.
...
\end{align*}
where the mean and variance of the negative binomial are defined as \[ \mathbb{E}[C_t] = \lambda_t \ \ \
\text{ and } \ \ \ \text{Var}[C_t] = \lambda_t + \frac{\lambda_t^2}{\phi}. \]
...
To standardise the magnitude of observations and forecasts across forecast locations we first normalised both weekly notified test positive cases and forecast test positive cases by the population in the forecast region scaled to be an incidence rate per 10,000 people.
...
p <- (
  ...
  theme(
    ...
    legend.justification = "centre", legend.justification = "vertical"
  )
...
Overall the surrogate model was relatively well calibrated at the 30%, 60% and 90% prediction interval, though with a tendency to slightly over cover, with empirical coverage of `r overall_coverage[range %in% "30% interval", Surrogate]`%, `r overall_coverage[range %in% "60% interval", Surrogate]`%, `r overall_coverage[range %in% "90% interval", Surrogate]`% respectively.
...
Rather than forecasting this ahead of time the Forecast ensembles generally only reacted to changes in the observed data indicating that variant information was not being used by forecasters.
Unlike the Forecast Hub ensemble the surrogate model can be, and indeed has been [@Abbott2021-ly], be extended to capture this. 
