...
The trend towards large-scale multi-team ensemble forecasting in infectious diseases has accelerated during the COVID-19 pandemic. Over 2020 and 2021, teams established COVID-19 Forecasting Hubs covering the US [@@Cramer2022-sp], Germany and Poland [@Bracher2021-wk], and Europe [@Sherratt2022-ue] (the latter two including authors of this study).
These projects were motivated partially to meet an increased need from stakeholders for reliable forecast models, but also by the perceived low quality of many forecasts otherwise being produced and publicised.

Whilst there is robust and consistent evidence that ensemble forecasts provide reliable and performant forecacasts across domains [@Reich2022-jo] they also have a range of downsides.
...
Here, we draw insights from ensemble forecasts produced and endorsed by the various COVID-19 forecast hubs, as well as our own forecasting work, to propose and evaluate a "surrogate" forecast model.
This seeks to emulate the observed behaviour of recent COVID-19 ensembles forecasting reported cases, using a minimal set of easily communicated assumptions, and limited computational resources with an easily generalised implementation.
...
In addition to this, we seek to provide a simple, and robust, forecasting system that can be easily resused by others, perhaps where a large scale forecast hub ensemble is not available, and be used as a platform for future research both into improving understanding of ensemble behaviour and increased forecast performance.
...
This means that, whilst non-parametric emulation has utility for allowing rapid exploration of the parameter space for epidemiological models [@Iskauskas2022-fz] utility, it may be less suitable for use in replicating epidemiological ensemble forecasts. 

In each of the COVID-19 Forecast Hubs, multiple independent teams contribute forecasts of reported cases, along with potentially other metrics, for a given set of locations, which the Hub collates and uses to produce an ensemble forecast.
Each Hub uses a similar approach focusing on forecasting by epidemiological week (i.e. Sunday to Saturday) over a time horizon of a few weeks horizon. In general, 4 main classes of forecast models are submitted.
This includes statistical forecasting models such as ARIMA models [@REF], mechanistic forecasting models based on the compartmental modelling framework and its generalisations such as individual-based models [@REF], semi-mechanistic approaches that blend statistical and mechanistic approaches [@REF], and human insight based forecast models that may also include elements of other methods [@REF].
Observed data are available and updated daily, and most submitted forecasts use this dataset, along with potentially other sources of real-time information, to produce daily forecasts.
Real-time evaluation has shown that each of these classes of models may perform well depending on the context and specific implementation.

We have contributed a range of forecasts to these collaborations involving statistical and semi-mechanistic models [@REF], and human insight-based forecasts [@REF].
With our semi-mechanistic forecast modelling, we have focussed on trying to model the underlying delays from infection through to symptom onset, and test positivity using non-exponential probability distributions whilst also modelling the complexity of daily, within the week, reporting periodicity [@REF].
With these models, we did not attempt to capture potential future interventions or known interventions not currently observed in the epidemiological data.
Our model-based Bayesian forecast methodology requires significant computational resources, running for approximately aan hour on a 16-core machine when producing forecasts for the European forecasting hub.
In contrast, our human insight models implicitly included unobserved interventions or other changes in the epidemiology and reporting process.
The human insight models were…[@REF]. Resources for the human insight models required an initial investment in building interactive forecasting infrastructure, followed by ongoing weekly participation from multiple independent forecasters.

Our forecast submissions have not clearly over- or under- performed other forecasts submitted to the forecasting hubs.
A clear weakness of our previous work has been a lack of robustness to large outliers in reporting and to changes in reporting patterns which has driven extremely poor performance at times.
We found that our human insight-based forecasts significantly outperformed our model-based forecasts on average and hypothesised that this may have been driven by including additional information not observed in the epidemiological data [@REF]. 

Considering multi-model ensembles, the respective Hubs produce an ensemble forecast each week comprising at least 3 independent forecasts [@REF].
To create the ensemble, as of September 2022, the US and European Hubs both used an unweighted median ensemble [@REF].
Evaluations of each of the COVID-19 Forecast Hubs have shown that these ensemble forecasts outperform most individually contributed forecasts whilst remaining generally robust to outliers in reporting [@REF].
Unlike our forecast submissions, the hub ensemble forecasts were typically robust to daily reporting artefacts. Similarly to our human insight forecasts, the ensembles also demonstrated some ability to forecast future changes in trends that were not present in the observed data.
This could indicate the inclusion of either human insight models, or assumptions about future interventions.
In general, the ensemble forecasts were more auto-correlated and less reactive to changes from stable or reducing case incidence to increasing incidence. On the other hand, this also meant that the ensemble was less likely to adopt short-term changes in incidence and hence produced better long-term forecasts. Finally, the ensemble forecast tended to produce sharper forecasts and have a tendency towards under-, versus over-, predicting.
...
This represents a parsimonious approach in that we encode our expectation that the growth rate should vary over time and allow this to influence the forecast but we include only a single lag term, reducing the complexity, and computational overhead of the model. 
...
This choice is motivated by our belief that the underlying transmission process is an exponential one and therefore a count error model (where variance is linked to the mean) is a sensible choice.
...
This means that non-parametric emulation is less suitable for use in epidemiological forecasting (while noting that epidemiological scenario analysis in-sample emulation using non-parametric methods may have a role [@Iskauskas2022-fz]). 
...
In our example set of locations, the absolute performance of the ensemble and the surrogate model was visually similar on the log scale in all locations at short forecast horizons though this varied by location.
Performance was most similar in Slovakia but in the United Kingdom and Germany in particular the surrogate model performed visibly worse for some forecast dates (Figure \@ref(fig:vis-forecasts)).
...
Breaking down the relative weighted interval score by forecast interval we observe that the surrogate modeel produces forecasts that differ most from the ensemble in the outer intervals and in particular the tails of the forecast (Figure \@ref(fig:plot-coverage) b).
This is true across forecast horizons but the magnitude of the difference increases.
If we consider the absolute error of the median forecast population weighted incidence we see that the median for the ensemble forecast was `r overall_ae_median[model == "Ensemble", round(Median, digits = 1)]` whilst the median for the surrogate forecast was `r overall_ae_median[model == "Surrogate", round(Median, digits = 1)]`.
When we consider the mean absolute error of the median we find that the ensemble forecast outperformed the surrogate forecast (`r overall_ae_median[model == "Ensemble", round(Mean, digits = 1)]` vs. `r overall_ae_median[model == "Surrogate", round(Mean, digits = 1)]`) which is comparable on the relative scale to the difference in performance observed using WIS.
This implies a similarly skewed distribution for the relaive point forecasts from the surrogate model point forecasts compared to the ensemble forecast which again  matches that observed when evaluating the full predictive distributions of both forecasts.
