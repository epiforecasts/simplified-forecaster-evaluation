...
The downsides of this approach is that it was not possible to update the surrogate model over time in response to initial evaluation or to explore other parameterisations that might be more successful.
...
Importantly in this study we focus on replicating the forecast hub ensembles observed beaviour rather than attempting to define an optimal forecast for forecast consumers.
This means that if the forecast ensemble is not optimal then we are optimising for the wrong target. 
That being said understanding what represents a useful forecast is difficult and an area of active research with a widespread assumption that large-scale ensembles of independent forecasts is the current state of the art. 
...
All data and code is available here: https://github.com/epiforecasts/simplified-forecaster-evaluation
