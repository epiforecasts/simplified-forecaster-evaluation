...
abstract: > 
  Multi-model and multi-team ensemble forecasts have become widely used in order to generate reliable short-term predictions of infectious disease spread. Notably, various public health agencies have used them to leverage academic disease modelling during the COVID-19 pandemic. However, ensemble forecasts are essentially black boxes and require extensive effort from numerous participating groups as well as a coordination team. In other fields, resource usage has been reduced by training simplified models that reproduce some of the observed behaviour of more complex models. Here we use observations of the behaviour of the European COVID-19 Forecast Hub ensemble combined with our own forecasting experience to identify a set of properties present in current ensemble forecasts. We then develop a parsimonious forecast model with the aim of mirroring these properties. We assess forecasts generated from this model in real time over a six-month period (15th of January 2022 to the 19th of July 2022) and for multiple European countries. We focus on forecasts of cases one to four weeks ahead and compare them to those by the European forecast hub ensemble. We find that the surrogate model behaves qualitatively similarly to the ensemble in many instances, though with increased uncertainty and poorer performance around periods of peak incidence. As a result, it falls short of the ensemble in terms of average performance. The performance differences, however, seem to be driven mainly by a subset of time points, and in terms of forecast calibration the proposed model actually outperforms the ensemble. We conclude that our simplified forecast model may have captured some of the dynamics of the hub ensemble, but more work is needed to understand the implicit epidemiological model that it represents. We moreover note that our findings may be driven by the choice of evaluation measure, which while widely used in the different forecast hubs, may not perfectly reflect forecast users' expectations or requirements. Our work will be of use to consumers of ensemble forecasts who wish to better understand their inherent properties, and to researchers aiming to generate reliable forecasts in the absence of the resources required for multi-model ensemble approaches.
...
These approaches have been inspired by the experience of other domains, for example, climate modelling [@IPCCreport], where ensembles of both multiple models and from multiple teams have a long history of providing forecasts that are trusted by stakeholders.
...
The most significant is the difficulty in interpreting them. This relates both to the underlying mechanisms for the forecasts they produce and to understanding if and when their behaviour can is desirable. 
...
Additional issues with multi-team collaborations include a lack of feedback to those contributing forecasts that would allow them to improve their forecast approaches, maintaining incentives for forecasters to continue to contribute and adjust their models to changing conditions [@Bosse2022-hm], and difficulty improving the quality of the ensemble [@Sherratt2022-ue].
Each of these issues may impact the long-term quality of the resulting ensemble forecasts and have implications for end-users. 
There has been limited critical feedback on forecasting ensembling projects for infectious disease epidemiology with little evaluation of the effort required to produce them compared to their benefits for improving forecast performance.
What evaluation there has been has mainly focussed on secondary data analysis.
...
Both the US and European Forecast Hubs were supported and received funding from public health agencies (the Center for Disease Control (CDC) and European Center for Disease Prevention and Control (ECDC) respectively) with their forecasts being adopted into the official output from these agencies.
However, less progress has been made in mitigating the downsides of these projects and improving access to the high-quality and robust forecasts they seek to generate to geographies without coverage or for other infectious diseases.

In this work, we draw insights from ensemble forecasts produced and endorsed by these Forecast Hubs, as well as our forecasting work, to propose and evaluate a "surrogate" forecast model.
This seeks to mitigate some of the downsides of recent COVID-19 ensembles forecasts by emulating their observed behaviour, using a minimal set of easily communicated and epidemiologically justifiable assumptions, and limited computational resources with an easily generalised implementation.
...
In addition to this, we seek to provide a simple, and robust, forecasting system that others can easily reuse, perhaps where a Forecast Hub ensemble is not available, and be used as a platform for future research both into improving understanding of ensemble behaviour and increased forecast performance.
...
This means that, whilst non-parametric emulation has utility for allowing rapid exploration of the parameter space for epidemiological models [@Iskauskas2022-fz; @Charles2022-ds], it may be less suitable for use in replicating epidemiological ensemble forecasts than the surrogate assumption driven framework we propose. 

In this study, we evaluate an initial attempt at a surrogate model to replicate the observed behaviour of current multi-team forecast ensembles based on a set of clear assumptions.
...
We defined anomalous data based on the implementation used by [@EuroHub] where a datapoint is considered anomalous if a future revision alters it by more than 5%.
...
Forecasters used a wide range of forecasting methods including statistical approaches, mechanistic models including deterministic ordinary differential equation models and agent-based models, semi-mechanistic models, expert elicitation models, and ensembles of multiple approaches [@EuroHub; @Bracher2022-ug; @Cramer2022-sp].
...
Each hub targets a range of metrics including test-positive reported cases, reported deaths, and hospitalisations has a specific geographic focus and asks for weekly forecasts (using epidemiological weeks i.e. Sunday to Saturday) over a time horizon of a few weeks.
...
We focus on reported cases as these represent the most common forecast target for COVID-19 forecast models [@Nixon2022-zv], they are often of the most direct interest due to being a leading indicator, and they are generally the most challenging to predict.
...
We have contributed a range of forecasts to these forecast collaborations, generally focused on semi-mechanistic statistical methods and human insight-based forecasts.
...
In comparison to our submitted forecasts, the ensemble forecasts were less reactive to changes in trend such as from stable or reducing case incidence to increasing incidence. On the other hand, this also meant that the ensemble was less likely to adopt short-term changes in incidence and hence produced better long-term forecasts. Finally, the ensemble forecast tended to produce sharper forecasts and have a tendency toward under vs over predicting. There is some suggestion that this was partially driven by the ensemble forecast having non-exponential uncertainty, unlike many infectious disease forecasting models

Based on these insights, here we define a model with similar, but simplified, epidemiological characteristics to our previous approaches to model-based forecasting [@Bosse2022-hm] to produce an ensemble-like performance without sacrificing interpretability.
...
\end{align*}
where the mean and variance of the negative binomial are defined as \[ \mathbb{E}[n] = \lambda_t \ \ \
\text{ and } \ \ \ \text{Var}[n] = \lambda_t + \frac{\lambda_t^2}{\phi}. \]
...
In our study period, incidence rates across European nations and in the UK were primarily driven by the spread of novel subvariants of concern related to the Omicron variant and changes in population susceptibility. Many countries, such as the UK, saw large BA.1 waves in January that then resulted in declining incidence rates through February (Figure \@ref(fig:vis-forecasts)).
...
On the natural scale the difference in performance was more marked, especially for periods of high incidence and at longer horizons (Figure \@ref(fig:vis-forecasts) a).
...
For both the ensemble and the surrogate, performance decreased as the forecast horizon increased with this being particularly noticeable for the surrogate model during periods of high incidence.
...
In contrast, the surrogate forecast was generally equally well calibrated across horizons with a tendency to over cover for intervals above the median.
At longer horizons, however, quantiles below the median became liable to under cover.
...
In comparison, the surrogate model was biased towards over prediction and this bias increased linearly with the forecast horizon.
...
This means that we are not subject to hindsight bias. 
...
The poor calibration of the forecast ensembles produced by median Hub ensembles has been noted repeatedly [@Cramer2022-sp; @Bracher2021-wk; @Sherratt2022-ue] but little progress has been made in understanding the causes or suggesting alternatives forecasts for policymakers to use.
...
A particular focus should be on understanding why our surrogate model was liable to overestimate peak incidence and what simply additional assumptions may be used to mitigate this.
...
SA and SF were funded by a Wellcome senior fellowship to SF (210758/Z/18/Z), KS and HG were funded by an ECDC grant to SF. JB acknowledges support from the Helmholtz Foundation via the SIM-610 CARD Information and Data Science Pilot Project.
