...
```{r empirical-coverage, eval = FALSE}
empirical_coverage <- scores |>
  DT(, horizon := as.character(horizon)) |>
  add_coverage(ranges = c(50, 90), by = c("model", "horizon")) |>
...
```{r vis-forecasts, fig.height = 9, fig.width = 9, fig.cap = "Forecasts of notified test positive cases (per 10,000 population) by epidemiological week in the United Kingdom, Germany, Slovakia, Italy, Poland, and Greece by forecast horizon (one to four weeks). Incidence rates are shown on a log scale. 30%, 60%, and 90% credible intervals are shown. The black line and points are the notified cases as of the date of data extraction rather than those available at the time."}
...
forecasts[location_name %in% locs] |>
  DT(, date := target_end_date) |>
    ...
    facet_grid(
      location_name ~ horizon) +
    aes(fill = model, col = model) +
    scale_y_log10(labels = comma, oob = squish, limits = c(0.1, 1000)) +
